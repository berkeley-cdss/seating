...
def student_photo(exam_student):
    ...
    if student_sid:
        ...
        import io
        photo = cache_store.get(cache_key_photo(student_sid))
        if photo is not None:
            return send_file(io.BytesIO(photo), mimetype='image/jpeg')
        from server.services.c1c import c1c_client
        photo = c1c_client.get_student_photo(student_sid)
        if photo is not None:
            cache_store.set(cache_key_photo(student_sid), photo, timeout=cache_life_photo)
            return send_file(io.BytesIO(photo), mimetype='image/jpeg')
